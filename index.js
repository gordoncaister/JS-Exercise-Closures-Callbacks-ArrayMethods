...
function processDuplicateFree(list, callback) {
  let newArray =  callback(list.filter((element, index) => list.indexOf(element) === index));
  return newArray;
 }
console.log(processDuplicateFree(['cat', 'cat', 'dog', 'leopard'],(arr) => arr))
...
function tallyUpDonations(runners) {
  return runners.reduce( (sum, x) => x.donation + sum,0);
}
